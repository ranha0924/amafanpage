...
(function() {
    ...
    function cacheElements() {
        ...
        elements.predictionCircuit = document.getElementById('predictionCircuit');
    }
    ...
    function updateCountdown() {
        ...
        if (typeof getNextRace !== 'function') {
            logger.warn('getNextRace 함수를 찾을 수 없습니다.');
            return;
        }

        const result = getNextRace();
        if (!result || !result.race) {
            logger.warn('다음 레이스 정보를 찾을 수 없습니다.');
            return;
        }
        ...
        if (elements.predictionCircuit) {
            elements.predictionCircuit.textContent = nextRace.circuit;
        }
        ...
        // 히어로 섹션 카운트다운 업데이트
        if (elements.countdownDays) elements.countdownDays.textContent = padZero(time.days);
