...
(function() {
    ...
    // ========================================
    // 안전한 Fetch 래퍼 (utils.js의 smartFetch 사용)
    // ========================================
    // safeFetch는 utils.js의 smartFetch로 통합됨
    // window.safeFetch = smartFetch (하위 호환성 유지)
     ...
     */
    function showGlobalAlert(message, type = 'info', title = '알림', options = {}) {
        ...
        // 동적 모달 생성
        showDynamicAlertModal(message, type, title, options);
    }

    function showDynamicAlertModal(message, type, title, options = {}) {
        // 기존 동적 모달 제거
        ...
        // allowHtml: true면 이미 이스케이프된 HTML을 그대로 렌더링 (정산 결과 등 내부 생성 메시지용)
        const safeMessage = options.allowHtml ? message : escapeHtmlSafe(message);
        ...
        modal.innerHTML = `
            ...
            <div class="alert-modal-content">
                ...
                <p class="alert-modal-message">${safeMessage}</p>
    ...
    // safeFetch는 utils.js의 smartFetch로 통합됨 (하위 호환성 유지)
